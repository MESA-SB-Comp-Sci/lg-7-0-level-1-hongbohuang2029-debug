# Short Response — LG 7.0: Responsive Units

Answer each question in 2–3 sentences. 

---

## Question 1 — Describe

What is the difference between a static unit like `px` and a responsive unit like `%` or `vh`?

Describe what makes a unit responsive and why that matters when building a website.
"px"is a fixed size it never change,it is great for small details that should never change,%and vh are the size of element change based on user screen size it has change every time.







---

## Question 2 — Explain

Look at these two CSS rules:

```css
.image {
  width: 400px;
}

.image {
  width: 50%;
}
```

Explain what happens to the image on a small screen with each rule. Why does one behave better than the other?
width50% is better,because 50% has change size based on user screen size ,on small screen width:50% has change size ,but 400px always don't change the size ,it always looks same,so 50%is better




